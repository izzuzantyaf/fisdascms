<div class="practicum-simulator-section col-span-full sm:col-span-6 lg:col-span-4 flex flex-col justify-between gap-4">
    <div class="practicum-simulator-info flex flex-grow items-center gap-4">
        ...
        <div id="simulator_visibility-{{ $id }}" class="visibility-toggler self-start flex-shrink-0 flex <?= $visibility ? 'bg-green-500 justify-end' : 'bg-gray-300' ?> rounded-full w-10 p-1 items-center gap-2 cursor-pointer">
            <input type="hidden" name="practicum_simulators[{{ $id }}][visibility]" id="simulator_visibility-{{$id}}" value="{{ $visibility }}" />
            <div id="simulator_visibility-{{$id}}" class="toggler-btn w-4 h-4 bg-white rounded-full"></div>
        </div>
    </div>
    <input type="url" name="practicum_simulators[{{ $id }}][link]" class="p-2 focus:ring-indigo-500 focus:border-blue-500 shadow-sm border border-gray-300 rounded-md" placeholder="Link" value="{{ $link }}">
</div>